...
(() => {
  ...
  const wikiInfo = title => {
    ...
    return wikiPending[title] = fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(title)}`).then(r => r.ok ? r.json() : null).then(j => {
      const thumb = j && j.thumbnail && j.thumbnail.source; const info = { img: thumb ? thumb.replace(/\/(\d+)px-/, '/720px-') : null, text: (j && j.type === 'standard' && j.extract) || '', url: (j && j.content_urls && j.content_urls.desktop && j.content_urls.desktop.page) || null };
      wikiCache[title] = info; try { localStorage.setItem(WK, JSON.stringify(wikiCache)); } catch (e) { } return info;
    }).catch(() => null);
  };
  const hydrate = root => { if (!root) return; root.querySelectorAll('[data-wiki]').forEach(el => wikiInfo(el.dataset.wiki).then(info => { if (!info || !el.isConnected) return; if (el.tagName === 'IMG') { if (info.img && !el.getAttribute('src')) { el.src = info.img; const box = el.closest('.photo, .cardPhoto'); if (box) box.hidden = false; } } else if (info.text) { el.textContent = info.text; el.hidden = false; const a = el.parentNode.querySelector('[data-wiki-link]'); if (a && info.url) { a.href = info.url; a.hidden = false; } } })); };
  ...
  const renderBook = () => {
    ...
    host.querySelectorAll('.photo img').forEach(img => { const fail = () => { const box = img.parentNode; img.remove(); if (box && !box.querySelector('img')) box.classList.add('none'); }; img.onerror = fail; if (img.complete && img.naturalWidth === 0 && img.src) setTimeout(() => { if (img.isConnected && img.complete && img.naturalWidth === 0) fail(); }, 0); });
