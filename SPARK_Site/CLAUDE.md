...
SPARK serves **any innovator who needs infrastructure to innovate**: ① governed **IaaS workspaces** (isolated project per use case — compute/storage/services, with or without AI) and ② **pre-approved LLM-as-a-service** on Ministry GPUs for AI builds. Messaging rules: always present **both offerings**; "AI Register" claims apply to **AI use cases** (not every workspace); budget guardrails and governance apply to **every** workspace; the framework narrative stays intact — for AI uses, the framework's clauses execute automatically. The capability-type select carries an "Infrastructure only — no AI service" option.
