...
- **NO M365 IN THE MINISTRY.** All SharePoint / Power Automate / Copilot / Microsoft-Forms references removed from every artifact. Register store = **central register list on the Ministry intranet** (v1 = .xlsx workbook); scheduled automation = **Cloud Scheduler → Cloud Function** on the existing GCP platform; feed drops = **/feeds Cloud Storage bucket**. Artifacts host centrally on the **Ministry intranet ("SPARK Playbook" under Digital Energy)** — not personally branded. Credit line: "Prepared and maintained by Abdulmajid Aldamigh — Planning".
