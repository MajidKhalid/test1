...
## GLOBAL RULE (Majid, R4, 1 Aug 2026): NO EM DASH
**Never use the em dash character "—" in anything produced for this project**: artifact copy (EN and AR), emails, announcements, UI strings, commit messages, PR text. Rewrite with a comma, colon, period, or parentheses instead. The portal/register/announcement sources are swept; legacy pages get swept as R4 touches them. (This memory file predates the rule; do not bulk-rewrite it, but write new entries without em dashes.)
...
- **R4 framing record (Majid, second round): "no one knows the framework and no one cares."** That quote is why the portal opens value-first; the framework lives on in internal artifacts. R4 also recorded: "it has no value" on the Leadership Hub deletion. The R4 chunk series on the handoff branch implemented the same brief independently; this branch's implementation superseded it at the v4 merge (see the v4 change report).
